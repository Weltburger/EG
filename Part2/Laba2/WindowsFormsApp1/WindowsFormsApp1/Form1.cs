...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        /* Метод получения кода положения точки относительно окна по алгоритму Коєна-Сазерленда */
        private uint code(double x, double y)
        {
            return (uint)((Convert.ToUInt16(x < xmin) << 3) | (Convert.ToUInt16(x > xmax) << 2) | (Convert.ToUInt16(y < ymin) << 1) | Convert.ToUInt16(y > ymax));
        }
        ...
        private void PictureBox1_Click(object sender, EventArgs e)
        {
            ...
            for (r = 0.5; r < 30.5; r += 0.5)
            {
                x2 = x0 + r * Math.Cos(phi0);
                y2 = y0 + r * Math.Sin(phi0);
                //for (i = 1; i <= 2; i++)
                //{
                    phi = phi0 + 1 * alpha;
                    x1 = x2 - 7; y1 = y2;
                    x2 = x0 + r * Math.Cos(phi) - 7; y2 = y0 + r * Math.Sin(phi);
            ...
            /*int i; double r, pi, alpha, phi0, phi, x0, y0, x1, y1, x2, y2;
            pi = 4.0 * Math.Atan(1.0);
            alpha = /*72*///90.0 * pi / 200.0; phi0 = 0.0; x0 = 0.0; y0 = 0.0;
            /* Вычерчивание границ окна */
            /*Draw(xmin, ymin, xmax, ymin); Draw(xmax, ymin, xmax, ymax);
            Draw(xmax, ymax, xmin, ymax); Draw(xmin, ymax, xmin, ymin);
            /* В пределах границ окна вычерчиваются 20 правильных концентрических пятиугольников */
            ...
            dc.DrawString(str, boldTimesFont, blueBrush, (pictureBox1.Size.Width -sizefText.Width) / 2, 0);*/
        }
    }
}
